...
class 'Katarina'
	...
	function Katarina:Menu()
		...
		self.menu = scriptConfig('Insdious Katarina', 'InsidiousKatarina')
	...
	function Katarina:Tick()
		...
		if not self.comp then
			if _G.AutoCarry then
				print("<font color=\"#FF0000\">[Insidious Katarina]:</font> <font color=\"#FFFFFF\">Found SAC Disabling SxOrb</font>")
			 ...
			 elseif _G.MMA_Loaded then
			 	print("<font color=\"#FF0000\">[Insidious Katarina]:</font> <font color=\"#FFFFFF\">Found MMA Disabling SxOrb</font>")
			 	if self.menu.orbwalk.General.Enabled and self.menu.orbwalk.General.Enabled == true then
			 		self.menu.orbwalk.General.Enabled = true
			 	end
			 	self.comp = true
		...
		if not self.ranUpdater then
			GetAsyncWebResult("raw.github.com" , self.versionLink, function(Data)
 	           ...
 	           if onlineVersion and onlineVersion > KatarinaVersion then
 	           		print("<font color=\"#FF0000\">[Insidious Katarina]:</font> <font color=\"#FFFFFF\">Found Update: </font> <font color=\"#FF0000\">"..KatarinaVersion.." > "..onlineVersion.." Updating... Don't F9!!</font>")
		...
		if self.needUpdate then
			DownloadFile(self.scriptLink, self.path, function()
                if FileExist(self.path) then
                    print("<font color=\"#FF0000\">[Insidious Katarina]:</font> <font color=\"#FFFFFF\">updated! Double F9 to use new version!</font>")
                end
            end)
            self.needUpdate = false
		end
